...
pub async fn filters(db: &State<DatabaseConnection>) -> Result<Json<Vec<filter::Model>>, Status> {
    ...
    Ok(Json(
        Filter::find()
            .all(db)
            .await
            .expect("Could not retrieve filters")
            .into_iter()
            .collect::<Vec<_>>(),
    ))
}
...
pub async fn update_filters(
    ...
    input: Json<Vec<filter::Model>>,
) -> Result<Json<Vec<filter::Model>>, Status> {
    ...
    Filter::delete_many()
    .exec(&txn)
    .await.expect("Could not delete filters.");

    let filter_models: Vec<filter::ActiveModel> = input.into_inner().into_iter().map(|f| {
